...
def _heuristic_classify(text: str):
    ...
    if any(k in t for k in ["infuriating", "angry", "outrage", "unacceptable"]):
        sentiment = "Angry"
    elif any(k in t for k in ["frustrated", "urgent", "blocked", "please help", "need urgent", "can't", "cannot", "asap"]):
    ...
    # ------------------- confidence -------------------
    conf = round(random.uniform(0.55, 0.95), 2)  # dynamic confidence for testing

    return {"topic": topic, "sentiment": sentiment, "priority": pr, "confidence": conf}
...
def classify_ticket(text: str):
    ...
    if OPENAI_KEY:
        try:
            ...
            prompt = f"""
You are a classifier. Output only JSON with keys: topic, sentiment, priority, confidence (0-1 numeric).
